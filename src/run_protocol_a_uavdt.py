"""Protocol A on UAVDT-2024-MOT, the second dataset (see REPORT.md / NOTES.md for why:
VisDrone turned out to be mostly near-hovering footage). Mirrors run_protocol_a.py
exactly, only the data-loading layer differs (uavdt_data_loader instead of data_loader).
Everything else (methods, metrics, static-track selection) is the same code, unchanged.

Run from the aerial_box_propagation/ directory: python3 src/run_protocol_a_uavdt.py
"""
...
def main():
    scenes = load_scenes()
    print(f"loaded {len(scenes)} UAVDT scenes", flush=True)

    all_records = []
    scene_summaries = {}

    for i, (scene_id, scene_frames) in enumerate(sorted(scenes.items())):
        width, height = scene_size(scene_frames)
        print(f"[{i+1}/{len(scenes)}][{scene_id}] {len(scene_frames)} frames, {width}x{height}", flush=True)
        ...
        print(f"  {len(key_frames)} key frames, {n_matched}/{len(step_info)} steps matched", flush=True)
        ...
        print(f"  {len(static_tracks)} qualifying static tracks", flush=True)
    ...
    with open(os.path.join(RESULTS_DIR, "uavdt_protocol_a_records.json"), "w") as f:
        json.dump(all_records, f)
    with open(os.path.join(RESULTS_DIR, "uavdt_scene_summaries.json"), "w") as f:
    ...
    print(f"\nwrote {len(all_records)} records, {n_tracks} unique static tracks, "
          f"{len(scenes)} scenes to {RESULTS_DIR}/", flush=True)
